...
This is a known Claude Code v2.1.83 bug ([#38651](https://github.com/anthropics/claude-code/issues/38651)), not a cc-guard issue. Any Stop hook — even `true` — causes `-p` (print mode) to return empty stdout.
...
**When this happens**:
- cc-guard hooks + another hook provider (e.g., project-level `.claude/settings.json` hooks)
- cc-guard hooks + manually added hooks in `~/.claude/settings.json` that match the same trigger

**When this does NOT happen**:
- cc-guard is the only hook source (default install)
