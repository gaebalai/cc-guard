...
#   CC_CACHE_DRIFT_LOG          Log path (default: ~/.cache/cc-guard/cache-creation-drift.jsonl)
...
#
# Inspect the log:
#   tail -50 ~/.cache/cc-guard/cache-creation-drift.jsonl
#   jq -s 'map(.cc_tokens) | add / length' ~/.cache/cc-guard/cache-creation-drift.jsonl

set -u

LOG="${CC_CACHE_DRIFT_LOG:-${HOME}/.cache/cc-guard/cache-creation-drift.jsonl}"
