...
# --- KOREAN PROHIBITIONS ---
run_test "Korean: TaskCreate 사용 금지 blocks TaskCreate" \
    2 "TaskCreate" \
    "\`TaskCreate\` 사용하지 마라."

run_test "Korean: TaskList 사용은 금지 blocks TaskList" \
    2 "TaskList" \
    "\`TaskList\` 사용은 금지한다."

run_test "Korean: 금지 도구 header with list blocks TaskCreate" \
    2 "TaskCreate" \
    "## 금지 도구
