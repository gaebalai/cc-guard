...
if echo "$QUESTION" | grep -qiE 'delete|삭제|drop|truncate|destroy|remove.*all|wipe|reset.*hard|force.*push|rm -rf'; then
    jq -n --arg q "$QUESTION" '{
        hookSpecificOutput: {
...
if echo "$QUESTION" | grep -qiE 'test|테스트|build|빌드|lint|format|check|확인|verify'; then
    jq -n --arg q "$QUESTION" '{
        hookSpecificOutput: {
            hookEventName: "PreToolUse",
            permissionDecision: "allow",
