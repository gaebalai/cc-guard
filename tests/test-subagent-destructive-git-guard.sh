...
# Test 7: Korean prompt with all 3 instructions, no warning
OUT=$(echo '{"tool_input":{"prompt":"git checkout / git reset / git restore 는 금지한다. stash 사용을 우선한다. git status 를 먼저 실행해 작업 트리 상태를 확인한 뒤, Tailwind class 를 rename 한다."}}' | bash "$HOOK" 2>&1)
RC=$?
assert_not_contains "Korean well-formed no warning" "$OUT" "destructive-git boundary"
assert_exit "Korean well-formed exit 0" "$RC" "0"
