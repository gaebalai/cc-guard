...
#      "do not use git checkout", "no git reset", "금지: git restore",
...
if ! printf '%s' "$PROMPT" | grep -qiE 'do not (use|run) (git )?(checkout|reset|restore)|no (git )?(checkout|reset|restore)|never (use|run) (git )?(checkout|reset|restore)|forbidden:? (git )?(checkout|reset|restore)|destructive git|파괴적인 git|금지:? (git )?(checkout|reset|restore)|git (checkout|reset|restore).{0,20}(금지|forbidden|prohibited)'; then
...
if ! printf '%s' "$PROMPT" | grep -qiE 'use git stash|stash before|stash first|reverse (sed|edit|change)|ask (the )?parent before|return to (the )?parent before any destructive|stash 사용|부모 에이전트에 확인'; then
...
if ! printf '%s' "$PROMPT" | grep -qiE 'check git status|run git status (first|before)|git diff (first|before)|verify working tree|working tree (state|status) (first|before)|git status를 먼저|작업 트리 상태를 (확인|검사)'; then
