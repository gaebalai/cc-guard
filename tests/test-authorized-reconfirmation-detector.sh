...
# --- Test 4: positive — Korean "권장" marker matches ---
INPUT=$(make_input \
    "migrate the table to the new schema" \
    "Should I migrate the schema now?" \
    '[{"label":"예","description":"진행 권장"},
      {"label":"아니오","description":"중지"}]')
run_hook "$INPUT" >/dev/null
if grep -q '"matched_word":"migrate"\|"matched_word":"schema"' "$LOG_TMP"; then
    assert_pass "positive: Korean 권장 marker triggers detection"
else
    cat "$LOG_TMP" >&2
    assert_fail "Korean marker case did not log"
fi
