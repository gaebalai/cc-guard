...
# License: MIT. Author: gaebalai (@gaebalai_dev).
#
# Companion to:
# - cc-guard (MIT, 772 PreToolUse/PostToolUse hooks)
#   https://github.com/gaebalai/cc-guard
#
    ...
    if (( SESSION_COUNT == 0 )); then
        ...
        if (( BACKUP_FILE_COUNT == 0 )); then
            report MEDIUM "No session backups present — protection against silent deletion is absent" \
                ...
                "Install cc-guard's session-backup-on-start.sh (writes to ~/.claude/session-backups) or set up rsync/restic. Book defense #5 + Chapter 3."
        else
...
if (( DENY_ENV > 0 )); then
    ...
    if (( AGENT_COUNT > 0 )); then
        report MEDIUM "Parent has $DENY_ENV .env-related deny rules + $AGENT_COUNT sub-agents — verify inheritance" \
            ...
            "Add an explicit PreToolUse hook that blocks Read/Edit/Write against .env paths in the sub-agent dispatch chain. cc-guard ships credential-exfil-guard.sh for this."
    else
...
• cc-guard — MIT-licensed PreToolUse/PostToolUse hook collection.
  https://github.com/gaebalai/cc-guard
