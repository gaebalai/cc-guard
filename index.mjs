...
// would naturally count off a calendar (UTC-based math drifts by a day for KST).
...
if (HELP) {
  console.log(`
  cc-guard — Make Claude Code safe for autonomous operation

  Quick Start:
    npx cc-guard              Install 8 safety hooks (30 sec)
    npx cc-guard --shield     Maximum safety — one command
    npx cc-guard --doctor     Diagnose hook problems
  ...
  Install & Configure:
    --install-example <name>       Install from 772 example hooks
  ...
  More: --create, --why, --replay, --score, --changelog, --analyze,
    ...
    npx cc-guard --quickfix   Auto-detect and fix common Claude Code problems
    npx cc-guard --stats      Block statistics and patterns report
    npx cc-guard --export     Export hooks config for team sharing
    npx cc-guard --import <file>  Import hooks from exported config
    npx cc-guard --help       Show this help
  ...
  More: https://github.com/gaebalai/cc-guard
  Find hooks: npx cc-hook-registry search <keyword>
  Test hooks: npx cc-hook-test <hook.sh>

  Token Checkup: https://claudecode.to/cc-guard/token-checkup.html
`);
  process.exit(0);
}
...
async function uninstall() {
  console.log();
  console.log(c.bold + '  cc-guard --uninstall' + c.reset);
  console.log();
...
function status() {
  console.log();
  console.log(c.bold + '  cc-guard --status' + c.reset);
  console.log();
  ...
  if (missing === 0) {
    ...
    console.log('  ' + c.dim + 'Run: npx cc-guard' + c.reset);
  }
  if (installedExamples.length > 0) {
  ...
  console.log('  ' + c.dim + '  Diagnose any unexpected charge: https://htmlpreview.github.io/?https://gist.githubusercontent.com/gaebalai/d3a0e2403cc4078aa0183400c137d824/raw/wrong-charge-diagnostic.html' + c.reset);
  console.log();
...
async function verify() {
  ...
  console.log();
  console.log(c.bold + '  cc-guard --verify' + c.reset);
  ...
  if (fail > 0) {
    console.log('  ' + c.red + fail + ' failures.' + c.reset + ' Run ' + c.blue + 'npx cc-guard' + c.reset + ' to reinstall.');
    process.exit(1);
  }
  console.log();
}
...
function examples() {
  ...
  console.log();
  console.log(c.bold + '  cc-guard --examples' + c.reset + (filter ? ' ' + filter : ''));
  ...
  if (filter && allFiltered.length > 0 && allFiltered.length <= 20) {
    ...
    for (const h of allFiltered) {
      console.log(c.dim + `    npx cc-guard --install-example ${h}` + c.reset);
    }
    console.log();
  }

  console.log(c.dim + '  Or install any single hook:' + c.reset);
  console.log(c.dim + '    npx cc-guard --install-example <name>' + c.reset);
  console.log(c.dim + '  Source: ' + c.blue + 'https://github.com/gaebalai/cc-guard/tree/main/examples' + c.reset);
  console.log();
}
...
async function audit() {
  console.log();
  console.log(c.bold + '  cc-guard --audit' + c.reset);
  ...
  if (!allHookCommands.match(/destructive|guard|block|rm|reset/i)) {
    risks.push({
      ...
      fix: 'npx cc-guard (installs destructive-guard)'
    });
  } else {
  ...
  if (!allHookCommands.match(/branch|push|main|master/i)) {
    risks.push({
      ...
      fix: 'npx cc-guard (installs branch-guard)'
    });
  } else {
  ...
  if (!allHookCommands.match(/secret|env|credential/i)) {
    risks.push({
      ...
      fix: 'npx cc-guard (installs secret-guard)'
    });
  } else {
  ...
  if (!allHookCommands.match(/database|wipe|migrate|prisma/i)) {
    risks.push({
      ...
      fix: 'npx cc-guard --install-example block-database-wipe'
    });
  } else {
  ...
  if (postHooks.length === 0) {
    risks.push({
      ...
      fix: 'npx cc-guard (installs syntax-check)'
    });
  } else {
  ...
  if (!allHookCommands.match(/dotfile|bashrc|protect/i)) {
    risks.push({
      ...
      fix: 'npx cc-guard --install-example protect-dotfiles'
    });
  }
  ...
  if (!allHookCommands.match(/scope|traversal|outside/i)) {
    risks.push({
      ...
      fix: 'npx cc-guard --install-example scope-guard'
    });
  }
  ...
  if (backslashCmds.length > 0) {
    risks.push({
      ...
      fix: 'npx cc-guard --uninstall && npx cc-guard@latest'
    });
  }
  ...
  if (!hasPermFixer && process.platform === 'win32') {
    risks.push({
      ...
      fix: 'npx cc-guard --install-example hook-permission-fixer'
    });
  }
  ...
  if (process.argv.includes('--fix') && risks.length > 0) {
    ...
    const { execSync } = await import('child_process');
    for (const r of risks) {
      if (r.fix.startsWith('npx cc-guard')) {
        try {
          const cmd = r.fix.replace('npx cc-guard', 'node ' + process.argv[1]);
    ...
    console.log();
    console.log(c.dim + '  Run with --fix to auto-apply: npx cc-guard --audit --fix' + c.reset);
  }
...
function learn() {
  console.log();
  console.log(c.bold + '  cc-guard --learn' + c.reset);
  ...
  if (!existsSync(logPath)) {
    ...
    console.log(c.dim + '  Install cc-guard first, then use Claude Code normally.' + c.reset);
...
exit 0`;
    ...
    if (!process.argv.includes('--apply')) {
      console.log();
      console.log(c.dim + '  Run with --apply to auto-create hooks: npx cc-guard --learn --apply' + c.reset);
    }
  }
...
async function fullSetup() {
  console.log();
  console.log(c.bold + c.green + '  cc-guard --full' + c.reset);
...
async function testHook(hookName) {
  ...
  if (!hookName) {
    console.log(c.bold + '  cc-guard --test-hook <name>' + c.reset);
    console.log(c.dim + '  Test any hook with sample inputs.' + c.reset);
    console.log();
    console.log('  Example: npx cc-guard --test-hook destructive-guard');
    return;
  }
...
async function saveProfile(name) {
  ...
  if (!name) {
    ...
    if (files.length === 0) {
      ...
      console.log(c.dim + '  Save: npx cc-guard --save-profile my-setup' + c.reset);
    } else {
      for (const f of files) {
        ...
        console.log(c.dim + `    Load: npx cc-guard --profile ${pName}` + c.reset);
      }
    }
    console.log();
    return;
  ...
  console.log(c.dim + `  Load: npx cc-guard --profile ${name}` + c.reset);
  console.log();
}
...
async function initProject() {
  console.log();
  console.log(c.bold + '  cc-guard --init-project' + c.reset);
...
async function suggest() {
  ...
  const { readdirSync } = await import('fs');
  console.log();
  console.log(c.bold + '  cc-guard --suggest' + c.reset);
  ...
  for (const r of risks) {
    ...
    if (r.hook) {
      ...
      if (isInstalled) {
        ...
        console.log(c.yellow + `     → Install: npx cc-guard --install-example ${r.hook}` + c.reset);
      }
    } else {
  ...
  if (unprotected.length > 0) {
    ...
    console.log(c.yellow + '  npx cc-guard --shield' + c.reset);
  } else {
...
async function why(hookName) {
  ...
  console.log();
  if (!hookName) {
    console.log(c.bold + '  cc-guard --why <hook-name>' + c.reset);
    console.log(c.dim + '  Show why a hook exists — the real incident that inspired it.' + c.reset);
    console.log();
    console.log('  Examples:');
    console.log(c.dim + '    npx cc-guard --why destructive-guard' + c.reset);
    console.log(c.dim + '    npx cc-guard --why token-budget-guard' + c.reset);
    console.log();
  ...
  console.log();
  console.log(c.dim + '  Install: npx cc-guard --install-example ' + name + c.reset);
  console.log();
}

async function replay() {
  console.log();
  console.log(c.bold + '  cc-guard --replay' + c.reset);
...
async function guard(description) {
  if (!description) {
    console.log();
    console.log(c.bold + '  cc-guard --guard "<rule>"' + c.reset);
    console.log(c.dim + '  Instantly enforce a rule — generates, installs, and activates a hook.' + c.reset);
    console.log();
    console.log('  Examples:');
    console.log(c.dim + '    npx cc-guard --guard "never touch the database"' + c.reset);
    console.log(c.dim + '    npx cc-guard --guard "block all sudo commands"' + c.reset);
    console.log(c.dim + '    npx cc-guard --guard "no force push"' + c.reset);
    console.log(c.dim + '    npx cc-guard --guard "warn before deleting files"' + c.reset);
    console.log();
    return;
  }
  ...
  console.log(c.dim + '  Remove: npx cc-guard --uninstall' + c.reset);
  console.log();
}

async function diffHooks(otherPath) {
  console.log();
  console.log(c.bold + '  cc-guard --diff-hooks' + c.reset);
  ...
  if (!otherPath) {
    ...
    if (existsSync(projectSettings) || existsSync(projectLocal)) {
      ...
      console.log(c.dim + '  Create with: npx cc-guard --team' + c.reset);
      console.log();
...
async function fromClaudeMd() {
  console.log();
  console.log(c.bold + '  cc-guard --from-claudemd' + c.reset);
  ...
  if (!claudeMdPath) {
    ...
    console.log(c.dim + '  Run: npx cc-guard --shield (creates one)' + c.reset);
    return;
  }
  ...
  if (matched.length === 0) {
    ...
    console.log(c.dim + '  For custom hooks: npx cc-guard --create "your rule"' + c.reset);
    return;
  }
  ...
  for (const m of matched) {
    ...
    if (!installed) {
      console.log(c.dim + `    npx cc-guard --install-example ${m.hook}` + c.reset);
    }
  }
  ...
  if (notInstalled.length > 0) {
    ...
    console.log(c.dim + '  npx cc-guard --shield' + c.reset);
  } else {
    console.log();
...
async function health() {
  ...
  if (!existsSync(HOOKS_DIR)) {
    ...
    console.log(c.dim + '  Run: npx cc-guard --shield' + c.reset);
    return;
  }
  ...
  if (issues > 0) {
    console.log();
    console.log(c.yellow + '  Fix issues: npx cc-guard --quickfix' + c.reset);
  }
...
async function migrateFrom(tool) {
  console.log();
  console.log(c.bold + '  cc-guard --migrate-from ' + (tool || '?') + c.reset);
  console.log();
  ...
  if (!tool) {
    ...
    for (const [id, info] of Object.entries(KNOWN_TOOLS)) {
      ...
      console.log(`    ${c.dim}npx cc-guard --migrate-from ${id}${c.reset}`);
      console.log();
    }
    return;
  }
  ...
  // Identify what cc-guard equivalents exist
  const replacements = [];
  for (const h of existingHooks) {
    ...
    if (replacement) {
      ...
      console.log(`    ${c.green}→${c.reset} cc-guard: ${replacement}`);
    } else {
  ...
  console.log(`  ${c.green}${replacements.length}${c.reset} hooks can be replaced with cc-guard equivalents`);
  console.log(`  ${c.dim}${existingHooks.length - replacements.length}${c.reset} hooks will be kept as-is`);
  console.log();
  console.log(c.dim + '  To apply: npx cc-guard --shield' + c.reset);
  console.log(c.dim + '  This installs cc-guard hooks alongside existing ones.' + c.reset);
...
async function team() {
  console.log();
  console.log(c.bold + '  cc-guard --team' + c.reset);
...
async function profile(level) {
  ...
  if (!level || !PROFILES[level]) {
    ...
    for (const [name, prof] of Object.entries(PROFILES)) {
      ...
      console.log(`  ${c.dim}npx cc-guard --profile ${name}${c.reset}`);
      console.log();
    }
      ...
      if (saved.length > 0) {
        ...
        for (const f of saved) {
          ...
          try {
            ...
            console.log(`  ${c.dim}npx cc-guard --profile ${sName}${c.reset}`);
            console.log();
          } catch {}
        }
      }
...
async function analyze() {
  ...
  const { readdirSync, statSync } = await import('fs');
  console.log();
  console.log(c.bold + '  cc-guard --analyze' + c.reset);
...
async function opus47() {
  console.log();
  console.log(c.bold + '  🚨  cc-guard --opus47' + c.reset);
  ...
  console.log(c.dim + '  Guide: https://claudecode.to/cc-guard/opus-47-survival-guide.html' + c.reset);
  console.log();
}
...
async function shield() {
  ...
  const { readdirSync } = await import('fs');
  console.log();
  console.log(c.bold + '  🛡️  cc-guard --shield' + c.reset);
...
`;
  ...
  console.log();
  console.log(c.dim + '  Verify: npx cc-guard --verify' + c.reset);
  console.log(c.dim + '  Status: npx cc-guard --status' + c.reset);
  console.log();
  console.log(c.dim + '  Burning tokens too fast? Free diagnosis:' + c.reset);
  console.log(c.blue + '  https://claudecode.to/cc-guard/token-checkup.html' + c.reset);
  console.log();
}
...
async function quickfix() {
  const { execSync } = await import('child_process');
  console.log();
  console.log(c.bold + '  cc-guard --quickfix' + c.reset);
    ...
    try {
      ...
      for (const name of critical) {
        if (allHookCmds.some(c => c.includes(name))) {
          ...
          console.log(c.dim + '    Fix: npx cc-guard' + c.reset);
          warnings++;
        }
      }
      if (safetyHooks === 3) {
  ...
  if (warnings > 0) {
    console.log();
    console.log(c.yellow + '  Run npx cc-guard to install missing safety hooks' + c.reset);
    console.log(c.yellow + '  Run npx cc-guard --doctor for detailed diagnosis' + c.reset);
  }
  console.log();
}
...
### Quick Actions
- Audit: \`npx cc-guard --audit\`
- Dashboard: \`npx cc-guard --dashboard\`
...
# Generated by: npx cc-guard --generate-ci
  ...
  safety-audit:
    ...
    steps:
      ...
      - name: Run safety audit
        id: audit
        run: |
          npx cc-guard --audit --json > /tmp/audit.json 2>&1 || true
...
`;
  ...
  console.log();
  console.log(c.bold + '  cc-guard --generate-ci' + c.reset);
  console.log();
...
async function migrate() {
  ...
  console.log();
  console.log(c.bold + '  cc-guard --migrate' + c.reset);
  ...
  const detections = [
    { pattern: /safety-net|cc-safety-net|SAFETY_LEVEL/i, project: 'claude-code-safety-net', replacement: 'npx cc-guard (destructive-guard, branch-guard)' },
    { pattern: /karanb192|block-dangerous-commands\.js/i, project: 'karanb192/claude-code-hooks', replacement: 'npx cc-guard (destructive-guard)' },
    { pattern: /hooks-mastery|disler|pre_tool_use\.py/i, project: 'disler/claude-code-hooks-mastery', replacement: 'npx cc-guard (multiple hooks)' },
    { pattern: /cchooks|from cchooks/i, project: 'GowayLee/cchooks', replacement: 'npx cc-guard (bash equivalents)' },
  ...
  if (found === 0 && suggestions.length === 0) {
    console.log(c.green + '  No migration needed. All hooks are cc-guard native.' + c.reset);
  } else {
    console.log(c.dim + '  Run npx cc-guard to install built-in replacements.' + c.reset);
  }
  console.log();
}
...
async function compare(hookA, hookB) {
  ...
  console.log();
  console.log(c.bold + '  cc-guard --compare' + c.reset);
  console.log();

  if (!hookA || !hookB) {
    console.log(c.red + '  Usage: npx cc-guard --compare <hook-a.sh> <hook-b.sh>' + c.reset);
    process.exit(1);
  }
...
function issues() {
  ...
  console.log();
  console.log(c.bold + '  cc-guard --issues' + c.reset);
...
async function dashboard() {
  ...
  function render() {
    ...
    console.log(pad(c.bold + 'cc-guard dashboard' + c.reset + '  ' + c.dim + timeStr + c.reset, W));
...
async function benchmark() {
  ...
  console.log();
  console.log(c.bold + '  cc-guard --benchmark' + c.reset);
...
function share() {
  console.log();
  console.log(c.bold + '  cc-guard --share' + c.reset);
  console.log();
  ...
  const url = 'https://claudecode.to/cc-guard/?config=' + encoded;
...
function diff(otherFile) {
  console.log();
  console.log(c.bold + '  cc-guard --diff' + c.reset);
  console.log();
...
async function lint() {
  console.log();
  console.log(c.bold + '  cc-guard --lint' + c.reset);
...
async function createHook(description) {
  console.log();
  console.log(c.bold + '  cc-guard --create' + c.reset);
...
async function stats() {
  ...
  console.log();
  console.log(c.bold + '  cc-guard --stats' + c.reset);
...
async function exportConfig() {
  console.log();
  console.log(c.bold + '  cc-guard --export' + c.reset);
  console.log(c.dim + '  Exporting hooks config for team sharing...' + c.reset);
  console.log();

  if (!existsSync(SETTINGS_PATH)) {
    console.log(c.red + '  No settings.json found. Run npx cc-guard first.' + c.reset);
    process.exit(1);
  }
  ...
  const outputFile = 'cc-guard-export.json';
  ...
  console.log(c.bold + '  npx cc-guard --import ' + outputFile + c.reset);
  console.log();
}

async function importConfig(file) {
  console.log();
  console.log(c.bold + '  cc-guard --import ' + file + c.reset);
...
async function watch() {
  ...
  console.log();
  console.log(c.bold + '  cc-guard --watch' + c.reset);
...
async function validateHooks() {
  ...
  console.log();
  console.log(c.bold + '  cc-guard --validate' + c.reset);
...
async function safeMode(enable) {
  ...
  console.log();
  if (enable) {
    console.log(c.bold + '  cc-guard --safe-mode' + c.reset);
    ...
    console.log(c.bold + '  cc-guard --safe-mode off' + c.reset);
...
async function compileRules(rulesFile) {
  console.log();
  console.log(c.bold + '  cc-guard --rules' + c.reset);
...
# Run: npx cc-guard --rules ${rulesFile}
...
# Rules: ${rules.length}
# Regenerate: npx cc-guard --rules ${rulesFile}
...
async function protect(targetPath) {
  ...
  console.log();
  console.log(c.bold + '  cc-guard --protect' + c.reset);
...
# Auto-generated by: npx cc-guard --protect ${targetPath}
...
async function simulate(command) {
  ...
  console.log();
  console.log(c.bold + '  cc-guard --simulate' + c.reset);
...
async function doctor() {
  ...
  console.log();
  console.log(c.bold + '  cc-guard --doctor' + c.reset);
  ...
  if (!existsSync(SETTINGS_PATH)) {
    ...
    try {
      ...
      console.log(c.dim + '    Fix: npx cc-guard --uninstall && npx cc-guard' + c.reset);
    }
            ...
            for (const entry of entries) {
              ...
              for (const h of hookList) {
                ...
                if (cmd.includes('\\')) {
                  ...
                  console.log(c.dim + '    Fix: npx cc-guard --uninstall && npx cc-guard@latest' + c.reset);
...
function scan() {
  console.log();
  console.log(c.bold + '  cc-guard --scan' + c.reset);
  ...
  console.log('  ' + c.dim + 'npx cc-guard' + c.reset + ' (8 built-in hooks)');
  for (const ex of examples) {
    console.log('  ' + c.dim + 'npx cc-guard --install-example ' + ex + c.reset);
  }
  ...
  if (existsSync(claudeMdPath)) {
    ...
    if (process.argv.includes('--apply')) {
      ...
      console.log();
      console.log(c.dim + '  Run with --apply to create: npx cc-guard --scan --apply' + c.reset);
    }
  }
...
async function main() {
  ...
  console.log();
  console.log(c.bold + '  cc-guard' + c.reset);
  ...
  console.log();
  console.log('  ' + c.dim + 'Verify:' + c.reset + '  npx cc-guard --verify');
  console.log('  ' + c.dim + 'More:' + c.reset + '    npx cc-guard --shield  (maximum safety)');
  console.log('  ' + c.dim + 'Diagnose:' + c.reset + ' https://claudecode.to/cc-guard/token-checkup.html');
  console.log('  ' + c.dim + 'Risk:' + c.reset + '     https://claudecode.to/cc-guard/risk-assessment.html');
  console.log('  ' + c.dim + 'Clusters:' + c.reset + ' https://claudecode.to/cc-guard/cluster-tracker.html  (7 open clusters, ~11,500 user reactions across 60+ issues)');
  console.log();
  ...
  console.log('  ' + c.dim + '    https://htmlpreview.github.io/?https://gist.githubusercontent.com/gaebalai/d3a0e2403cc4078aa0183400c137d824/raw/wrong-charge-diagnostic.html' + c.reset);
  console.log();
  console.log('  ' + c.dim + 'Feedback:' + c.reset + ' https://github.com/gaebalai/cc-guard/discussions  (questions / patterns that worked)');
  console.log('  ' + c.dim + 'Bug:' + c.reset + '      https://github.com/gaebalai/cc-guard/issues/new/choose  (false positives / install issues)');
  console.log();
}
