...
#
#   Format 2 — Korean prose:
#     `TaskCreate` 사용 금지
#     `TaskList` 사용은 금지된다
#
#   Format 3 — list under a "Forbidden Tools" / 금지 header (within 20 lines):
#     ## Forbidden Tools
#     - `TaskCreate`
#     - `TaskList`
#
...
#   Pass 1 — prose patterns matching "do not use `X`" / "금지" / etc.
#   Pass 2 — list items under headers that mention forbidden / prohibited / 금지.
FORBIDDEN_LIST=""

# Pass 1: prose prohibitions
PROSE=$(grep -iE '(do[ -]not (use|call)|never (use|call)|금지|사용 금지|사용하지 말|사용은 금지)' "$CLAUDEMD" 2>/dev/null || true)
if [ -n "$PROSE" ]; then
...
# Pass 2: list items under "Forbidden Tools" / "Prohibited Tools" / "금지" headers
# (only the 20 lines following the header are scanned)
HEADER_LINES=$(grep -inE '^#+ +(forbidden tools|prohibited tools|금지 도구|사용 금지)' "$CLAUDEMD" 2>/dev/null | cut -d: -f1 || true)
