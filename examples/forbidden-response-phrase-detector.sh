...
#     `네, 맞습니다` 사용은 금지된다
#
#   Format 3 — list under a "Forbidden Response Phrases" / 금지 header:
...
PROSE=$(grep -iE '(do[ -]not (open|start|begin|use|say)|never (say|use|open|begin)|답변에|사용 금지|사용은 금지|답변 서두|응답 서두|filler|opening)' "$CLAUDEMD" 2>/dev/null || true)
if [ -n "$PROSE" ]; then
...
# Pass 2: list items under "Forbidden Response Phrases" / "Forbidden Phrases" / 금지 headers
HEADER_LINES=$(grep -inE '^#+ +(forbidden (response )?phrases|forbidden replies|prohibited (response )?phrases|filler phrases|금지 응답|응답 금지)' "$CLAUDEMD" 2>/dev/null | cut -d: -f1 || true)
