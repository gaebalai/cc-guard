...
# --- Test 6: Korean markers (반드시 / 절대 / etc.) are surfaced ---
mkdir -p "$TMP_MEMORY/korean"
cat > "$TMP_MEMORY/korean/feedback-kr.md" <<'EOF'
# 한국어 규칙

절대 규칙: 게시 전에 반드시 직접 사실 확인을 수행한다.
승인 없이 구매 페이지 변경은 하지 마라.
EOF
output=$(echo '{}' | CC_STANDING_RULE_MEMORY_DIR="$TMP_MEMORY/korean" bash "$HOOK")
if echo "$output" | grep -q "절대"; then
    echo "  PASS: Korean 절대 marker is surfaced"
    PASS=$((PASS + 1))
else
    echo "  FAIL: Korean 절대 not in output: $output"
    FAIL=$((FAIL + 1))
fi
